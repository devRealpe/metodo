...
export class NotificationsPanelComponent implements OnInit, OnDestroy {
  ...
  onNotificationClick(notification: Notification): void {
    ...
    if (notification.link) {
      ...
      // Eliminar prefijo del proyecto si el backend lo incluye en el link
      const baseSegment = 'internacionalizacion';
      if (link.startsWith('/' + baseSegment + '/')) {
        link = link.substring(('/' + baseSegment).length);
      } else if (link.startsWith(baseSegment + '/')) {
        link = '/' + link.substring((baseSegment + '/').length);
      }
